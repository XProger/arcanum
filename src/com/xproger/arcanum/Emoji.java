...
public class Emoji {
	public static Bitmap smileSheet;
	
	public static class Keyboard implements OnClickListener {
		private PopupWindow popup = null;
		private Context context;
		private View above, scroll;
		private ViewPager pager;
		private EditText edit;
		private int height;
		private ImageView btn[];
		
		public Keyboard(Context context, View aboveView, EditText editView, final CheckBox checkBox) {
			...
			above.getViewTreeObserver().addOnGlobalLayoutListener(
				new ViewTreeObserver.OnGlobalLayoutListener() {
					@Override
					public void onGlobalLayout() {
						Rect rect = new Rect();
						above.getWindowVisibleDisplayFrame(rect);
						height = above.getRootView().getHeight() - rect.bottom;
						if (popup == null)
							return;
						if (popup.isShowing()) {
							if (height > 0) {
								if (height != popup.getHeight()) {
									popup.dismiss();
									popup.setHeight(height);
									popup.showAtLocation(above, Gravity.BOTTOM, 0, 0);
								}
							} else
		...
		private void init() {
	        ...
	        View view = inflater.inflate(R.layout.emoji_keyboard, null, false);
	        
	        final View panel = view.findViewById(R.id.emoji_panel);
	        //panel.findViewById(R.id.emoji_recent);    
	        
	        btn = new ImageView[7];
	        for (int i = 0; i < 7; i++) {
	        	btn[i] = (ImageView)panel.findViewWithTag(Integer.toString(i));
	        	btn[i].setOnClickListener(this);
	        }
	        
	        scroll = view.findViewById(R.id.emoji_scroll);
	        
	    	pager = (ViewPager)view.findViewById(R.id.emoji_pager);
	    	pager.setAdapter(new Adapter.EmojiPagerAdapter(new OnClickListener() {
				@Override
				public void onClick(View view) {				
					long code = (Long)view.getTag();
					int pos = edit.getSelectionStart();
					Editable text = edit.getText();
					String codeStr = Emoji.getCodeStr(code);				
					text.insert(pos, codeStr);		
				}
	    	})); 
	    	...
	    	pager.setOnPageChangeListener(new OnPageChangeListener() {
				...
				public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {					
					...
					for (int i = 0; i < 6; i++) {
						float alpha = 1.0f - Math.min(Math.abs(p - i), 1.0f);	
						
						btn[i].getDrawable().setAlpha((int)((0.5f * alpha + 0.5f) * 255));
						btn[i].invalidate();
						//btn[i].setAlpha(0.5f * alpha + 0.5f);
					}					
			...
			popup = new PopupWindow(view, LayoutParams.MATCH_PARENT, 0, false);	    	
			updateScroll(0);
		}
		
		private void updateScroll(float pos) {
			float w = above.getWidth() / 7;		
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams((int)w, LayoutParams.MATCH_PARENT);
			params.gravity = Gravity.LEFT | Gravity.TOP;
			params.setMargins((int)(pos * w), 0, 0, 0);			
		...
		public void setVisible(boolean visible) {
			if (visible && (popup == null || !popup.isShowing())) {	
				init();
	...
	public static class Smile extends Drawable {
		private static Paint paint;
		
		public static int SMILE_SIZE;
		public int index;
		private int size;
		
		public static Smile getSmile(int index) {
			return new Smile(index, 1.0f);
		}
		
		Smile(int index, float scale) {
			this.index = index;
			size = (int)(SMILE_SIZE * scale);
		...
		public void draw(Canvas canvas) {
			if (smileSheet == null) return;
			
			int x = (index % 32) * 32;
			int y = (index / 32) * 32;
			Rect src = new Rect(x, y, x + 32, y + 32);
			Rect dst = new Rect(0, 0, size, size);
			
			canvas.drawBitmap(smileSheet, src, dst, paint);
		}
    ...
    public static SparseIntArray smileSet = new SparseIntArray();
    
    public static int getIndex(long code) {
    	return smileSet.get((int)code);
    }

    public static String getCodeStr(long code) {
    	if ((code & 0xffff0000L) != 0L)	// 4 bytes code
    		return String.valueOf(new char[]{(char)((code >> 16) & 0xffff), (char)(code & 0xffff)});
    	else							// 2 byte code
    		return String.valueOf((char)(code & 0xffff));
    }
    ...
    private static void initSheet() {
    	...
    	new Thread(new Runnable() {
			...
			public void run() {
		    	...
		    	Bitmap mask = BitmapFactory.decodeResource(Main.main.getResources(), R.drawable.emoji_mask, opt);		    	
		    	opt.inPreferredConfig = Bitmap.Config.RGB_565;
		    	Bitmap bmp = BitmapFactory.decodeResource(Main.main.getResources(), R.drawable.emoji, opt);
		    	
		    	Bitmap tmp = Bitmap.createBitmap(bmp.getWidth(), bmp.getHeight(), Bitmap.Config.ARGB_8888);
		    	...
		    	if (Main.main != null && Main.main.mainView != null)
		    		Main.main.mainView.post(new Runnable() {
						@Override
						public void run() {
							Main.main.mainView.invalidate();
						}
		    		});
    ...
    public static void init() {    	
    	Smile.SMILE_SIZE = (int)Common.dipToPixels(20);
    	initSheet();
    	...
    	for (int i = 0; i < data.length; i++)
    		for (int j = 0; j < data[i].length; j++)
    			smileSet.put((int)data[i][j], idx++);
    }

    public static void replace(Editable text) {    	 
    	for (ImageSpan span : text.getSpans(0, text.length(), ImageSpan.class))
    		text.removeSpan(span);    		    	
    	
    	int i = 0, len = text.length(), code;
    	Integer value;    	
    	
    	while (i < len) {    		
    		code = text.charAt(i);
    		if ((value = smileSet.get(code, -1)) == -1) {
	    		if (i + 1 < len) {	    			
	    			code = (code << 16) | text.charAt(i + 1);
	    			if ((value = smileSet.get(code, -1)) != -1) {
    					text.setSpan(new ImageSpan(new Smile(value, 1.0f), ImageSpan.ALIGN_BOTTOM), i, i + 2, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
	    				i++;
	    			}	    				    			
	    		}
    		} else
    			text.setSpan(new ImageSpan(new Smile(value, 1.0f), ImageSpan.ALIGN_BOTTOM), i, i + 1, Spannable.SPAN_INCLUSIVE_INCLUSIVE);
    		i++;
    	}
    }    
 ...
 /*
    public static String replace(String text) {    	 
    	...
    	while (i < len) {
    		code = text.charAt(i);
    		value = smileSet.get(code);
    		
    		if (value != null) {
    			sb.append("<img src=\"" + value.toString() + "\"/>");
    		} else
	    		if (i + 1 < len) {	    			
	    			code = (code << 16) | text.charAt(i + 1);
	    			value = smileSet.get(code);
...
*/
    public static  long[][] data={new long[]{},
		new long[]{0x00000000D83DDE04L, 0x00000000D83DDE03L, 0x00000000D83DDE00L, 0x00000000D83DDE0AL, 0x000000000000263AL, 0x00000000D83DDE09L, 0x00000000D83DDE0DL,
		...
		new long[]{0x00000000D83DDC36L, 0x00000000D83DDC3AL, 0x00000000D83DDC31L, 0x00000000D83DDC2DL, 0x00000000D83DDC39L, 0x00000000D83DDC30L, 0x00000000D83DDC38L, 0x00000000D83DDC2FL,
		...
		new long[]{0x00000000D83CDF8DL, 0x00000000D83DDC9DL, 0x00000000D83CDF8EL, 0x00000000D83CDF92L, 0x00000000D83CDF93L, 0x00000000D83CDF8FL, 0x00000000D83CDF86L, 0x00000000D83CDF87L,
		...
		new long[]{0x00000000D83CDFE0L, 0x00000000D83CDFE1L, 0x00000000D83CDFEBL, 0x00000000D83CDFE2L, 0x00000000D83CDFE3L, 0x00000000D83CDFE5L, 0x00000000D83CDFE6L, 0x00000000D83CDFEAL,
		...
		new long[]{0x00000000003120E3L, 0x00000000003220E3L, 0x00000000003320E3L, 0x00000000003420E3L, 0x00000000003520E3L, 0x00000000003620E3L, 0x00000000003720E3L, 0x00000000003820E3L,
